...
import "./CustomTrendingCarousel.css"; // We'll put the CSS here

interface CustomTrendingCarouselProps {
  articles: (Article & { author?: { name: string | null } })[];
}

export default function CustomTrendingCarousel({ articles }: CustomTrendingCarouselProps) {
  const rootRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (!rootRef.current || articles.length === 0) return;

    // Translated the user's JS into a local class/closure
    class MzaCarousel {
      root: HTMLElement;
      ...
      constructor(root: HTMLElement, opts = {}) {
        ...
        this.prevBtn = root.querySelector(".mzaCarousel-prev") as HTMLElement;
        this.nextBtn = root.querySelector(".mzaCarousel-next") as HTMLElement;
        ...
        // Force isFF to false so 3D transform is always used
        this.isFF = false;
    ...
    const mza = new MzaCarousel(rootRef.current, { transitionMs: 900 });
  ...
  return (
    <div className="mzaCarousel" id="mzaCarousel" aria-roledescription="carousel" aria-label="Featured cards" ref={rootRef}>
      <div className="mzaCarousel-viewport" tabIndex={0}>
        <div className="mzaCarousel-track">
          {articles.map((article, i) => (
            <article key={article.id} className="mzaCarousel-slide" role="group" aria-roledescription="slide" aria-label={`${i + 1} of ${articles.length}`}>
              <div className="mzaCard" style={{ "--mzaCard-bg": `url('${article.coverImage || '/images/default-article.jpg'}')` } as any}>
                <header className="mzaCard-head mzaPar-1">
                  <h2 className="mzaCard-title text-white">{article.title}</h2>
                  <p className="mzaCard-kicker">{article.category || "Berita Hukum"}</p>
                </header>
                ...
                <footer className="mzaCard-actions mzaPar-3">
                  <Link href={`/blog/${article.slug}`}>
                    <button className="mzaBtn">Baca Selengkapnya</button>
                  </Link>
                </footer>
              </div>
            </article>
